...
# this name is used to make new directory as name that can be seen in C:\tmp\movielens-data\wide_deep_buffer(M)
_BUFFER_SUBDIR = "wide_deep_buffer"
# this is one dictionary that use name of header as key and parsing value of that header in data base as value(M)
_FEATURE_MAP = {
    # tf.FixedLenFeature is method from tensorFlow that parsing fixed-length input arg_1 is shape of input(M)
...
# I don't know but need to know according embedding layer
...
def build_model_columns(dataset):
    ...
    # this function is used when you are sparse input and need to dens it to feed it to the DNN(M)
    # max_norm is one approach for regularization of wight of neuron(M)
...
def _buffer_path(data_dir, dataset, name):
    # with this devise "{}_{}_buffer".format(dataset, name) we have result like dataset_name_buffer(M)
...
if __name__ == "__main__":
    ...
    flags_core.set_defaults(dataset="ml-20m")
    absl_app.run(main)
